<template>
    <AuthenticatedLayout title="Dashboard">
        ...
        <div class="p-6 overflow-hidden bg-white rounded-md shadow-md dark:bg-dark-eval-1">
            ...
            <!-- Main modal -->
            <div v-if="this.isOpen" id="defaultModal" tabindex="-1" aria-hidden="true" class="overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full flex justify-center items-center">
                <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div class="p-6 space-y-6">
                           <form @submit.prevent="store">
                            <div>
                                ...
                                <input v-model="storePermissionForm.name" type="text" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Ex: create" required>
                            </div>
                           </form>
                        </div>
                        <!-- Modal footer -->
                        <div class="flex items-center p-6 space-x-2 rounded-b border-t border-gray-200 dark:border-gray-600">
                            ...
                            <Button @click="store()" :disabled="storePermissionForm.processing || storePermissionForm.name === null" variant="success" class="items-center gap-2 max-w-xs"
                                v-slot="{ iconSizeClasses }">
                                <CheckCircleIcon v-if="!storePermissionForm.processing" aria-hidden="true" :class="iconSizeClasses" />
                                <div v-if="storePermissionForm.processing" role="status">
                            ...
                            <Button v-show="editMode" variant="info" class="items-center gap-2 max-w-xs"
                                v-slot="{ iconSizeClasses }">
                                <PencilAltIcon aria-hidden="true" :class="iconSizeClasses" />
                                <span>Atualizar</span>
                            </Button>
            ...
            <div v-if="this.isOpen" class="absolute z-40 inset-0 opacity-50 bg-black"></div>
        </div>
  ...
  export default {
    ...
    data() {
        return {
            isOpen: false,
            editMode: false,
            storePermissionForm: this.$inertia.form({name: null})
        }
    },
    ...
    methods: {
        ...
        resetForm() {
            this.storePermissionForm.reset();
        },
        store() {
            this.storePermissionForm.post("permissions", {
                onSuccess: () => {
                    this.resetForm();
